...
class InteractiveGP():
    def __init__(self, X, Y, kernel, y_lim, alpha=1e-10, n_restarts_optimizer=20):
        ...
        self.gp = GaussianProcessRegressor(kernel=kernel, alpha=alpha, n_restarts_optimizer=n_restarts_optimizer)#make GP and train
        #alpha就是添加到协方差矩阵对角线上的值，n_restarts_optimizer规定了优化过程的次数
    def fit(self):

        gp_ = self.gp.fit(self.X, self.Y)
#这是拟合高斯过程回归的步骤，data[:,:-1]获取前两列元素值，data[:,-1]获取后两列元素的值
    ...
    def predict(self, x, return_std=True):#np.r_是按列连接两个矩阵，就是把两矩阵上下相加，要求列数相等，类似于pandas中的concat()
